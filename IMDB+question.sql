...
select year, count(date_published) as number_of_movies from movie group by year;
...
select n.name, total_votes, count(m.id) as movie_count,
...
select name as actress_name, sum(total_votes) as total_votes, count(rm.movie_id) as movie_id, round(sum(avg_ratingtotal_votes)/sum(total_votes),2) as actress_avg_rating,
